document.addEventListener('DOMContentLoaded', function() {
    ...
    const ctx = document.getElementById('georgiaChart').getContext('2d');
    let georgiaChart;
        ...
        fetch('/api/pollings')
            ...
            .then(pollings => {
                ...
                pollings.forEach(polling => {
                    if (polling.territory.territory=='Georgia') {  
        ...
        fetch('/api/pollings')
            ...
            .then(pollings => {
                const filteredPollings = pollings.filter(polling =>
                    polling.pollster.idpollster == pollsterId && polling.territory.territory=='Georgia'
                );
                ...
                if (georgiaChart) {
                    georgiaChart.destroy();
                }

                georgiaChart = new Chart(ctx, {
                    type: 'line',
                    data: {
                        labels: allLabels,
                        datasets: datasets
