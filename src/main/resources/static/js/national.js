document.addEventListener('DOMContentLoaded', function() {
    ...
    function populatePollsterDropdown() {
        fetch('/api/pollings')
            ...
            .then(pollings => {
                const validPollsterIds = new Set();
                pollings.forEach(polling => {
                    if (polling.territory.territory=='National') {
                        validPollsterIds.add(polling.pollster.idpollster);
                    }
                });
                ...
                return fetch('/api/pollsters')
                    ...
                    .then(pollsters => {
                        const filteredPollsters = pollsters.filter(pollster =>
                            validPollsterIds.has(pollster.idpollster)
                        );
        ...
        fetch('/api/pollings')
            ...
            .then(pollings => {
                const filteredPollings = pollings.filter(polling =>
                    polling.pollster.idpollster == pollsterId && polling.territory.territory=='National'
                );
