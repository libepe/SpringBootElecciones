document.addEventListener('DOMContentLoaded', function() {
    ...
    const ctx = document.getElementById('arizonaChart').getContext('2d');
    let arizonaChart;
        ...
        fetch('/api/pollings')
            ...
            .then(pollings => {
                ...
                pollings.forEach(polling => {
                    if (polling.territory.territory=='Arizona') {  
        ...
        fetch('/api/pollings')
            ...
            .then(pollings => {
                const filteredPollings = pollings.filter(polling =>
                    polling.pollster.idpollster == pollsterId && polling.territory.territory=='Arizona'
                );
                ...
                if (arizonaChart) {
                    arizonaChart.destroy();
                }

                arizonaChart = new Chart(ctx, {
                    type: 'line',
                    data: {
                        labels: allLabels,
                        datasets: datasets
