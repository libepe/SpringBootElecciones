document.addEventListener('DOMContentLoaded', function() {
    ...
    const ctx = document.getElementById('michiganChart').getContext('2d');
    let michiganChart;
        ...
        fetch('/api/pollings')
            ...
            .then(pollings => {
                ...
                pollings.forEach(polling => {
                    if (polling.territory.territory == 'Michigan') {  
        ...
        fetch('/api/pollings')
            ...
            .then(pollings => {
                const filteredPollings = pollings.filter(polling =>
                    polling.pollster.idpollster == pollsterId && polling.territory.territory == 'Michigan'
                );
                ...
                if (michiganChart) {
                    michiganChart.destroy();
                }

                michiganChart = new Chart(ctx, {
                    type: 'line',
                    data: {
                        labels: allLabels,
                        datasets: datasets
