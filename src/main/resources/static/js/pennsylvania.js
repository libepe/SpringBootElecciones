document.addEventListener('DOMContentLoaded', function() {
    ...
    const ctx = document.getElementById('pennsylvaniaChart').getContext('2d');
    let pennsylvaniaChart;
        ...
        fetch('/api/pollings')
            ...
            .then(pollings => {
                ...
                pollings.forEach(polling => {
                    if (polling.territory.territory == 'Pennsylvania') {
        ...
        fetch('/api/pollings')
            ...
            .then(pollings => {
                const filteredPollings = pollings.filter(polling =>
                    polling.pollster.idpollster == pollsterId && polling.territory.territory == 'Pennsylvania'
                );
                ...
                if (pennsylvaniaChart) {
                    pennsylvaniaChart.destroy();
                }

                pennsylvaniaChart = new Chart(ctx, {
                    type: 'line',
                    data: {
                        labels: allLabels,
                        datasets: datasets
